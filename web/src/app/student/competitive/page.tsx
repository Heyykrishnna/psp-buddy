...
export default function CompetitivePage() {
  ...
  const loadLeaderboard = useCallback(async (tf: LbTimeframe) => {
    setLbLoading(true);
    try {
      const data = await apiFetch<any[]>(`/competitive/leaderboard?timeframe=${tf}&limit=50`);
      setLeaderboard(data || []);
    } catch {}
    setLbLoading(false);
  }, []);
  ...
  const streakDates = new Set(streakCalendar.map((d: string) => new Date(d).toDateString()));
      ...
      <div className="min-h-screen bg-[#F5F5F7]">
        ...
        <header className="h-14 bg-white border-b border-zinc-200 px-6 flex items-center justify-between sticky top-0 z-20">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-bold text-zinc-900">Competitive Hub</h1>
          </div>
        ...
        <nav className="bg-white border-b border-zinc-200 px-6 flex items-center gap-1">
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`py-3.5 px-4 text-xs font-bold uppercase tracking-wider transition-all cursor-pointer border-b-2 ${
                tab === t.id
                  ? "border-[#111111] text-zinc-900"
        ...
        <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
          {loading ? (
            <div className="flex items-center justify-center py-20 text-zinc-400 text-sm gap-2">
              <RefreshCw className="w-4 h-4 animate-spin" /> Loading competitive data...
            </div>
          ) : (
            <>
              {/* ── OVERVIEW ─────────────────────────────────────────────────── */}
                  ...
                  {profile && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {[
                        ...
                        >
                          <div className={`w-10 h-10 ${stat.bg} border ${stat.border} rounded-xl flex items-center justify-center shrink-0`}>
                    ...
                    <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-bold text-zinc-900">Daily Challenge</div>
                            <div className="text-[11px] text-zinc-400">Problem of the Day</div>
                          </div>
                        </div>
                      ...
                      {dailyChallenge ? (
                        <div className="space-y-3">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border inline-block ${DIFF_COLOR[dailyChallenge.problem?.difficulty || "EASY"]}`}>
                            {dailyChallenge.problem?.difficulty}
                          </span>
                          <div className="font-bold text-zinc-900 text-sm">{dailyChallenge.problem?.title}</div>
                          <div className="text-xs text-zinc-500">{dailyChallenge.totalSolved} people solved today</div>
                          {profile?.dailyChallenge?.completed ? (
                            ...
                            <button
                              onClick={() => router.push(`/student/playground?problemId=${dailyChallenge.problemId}`)}
                        ...
                        <div className="text-xs text-zinc-400 italic">No challenge configured for today.</div>
                      )}
                    </div>
                    ...
                    <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-bold text-zinc-900">Weekly Challenge</div>
                            <div className="text-[11px] text-zinc-400">This week&apos;s goal</div>
                          </div>
                        </div>
                      ...
                      {weeklyChallenge ? (
                        <div className="space-y-3">
                          <div className="font-bold text-zinc-900 text-sm">{weeklyChallenge.title}</div>
                          <div className="text-xs text-zinc-500">{weeklyChallenge.description}</div>
                          {(() => {
                            ...
                            return (
                              <div className="space-y-1.5">
                                <div className="flex justify-between text-xs text-zinc-600">
                                  <span>{progress} / {target} problems</span>
                        ...
                        <div className="text-xs text-zinc-400 italic">No weekly challenge active.</div>
                      )}
                    </div>
                  </div>
                  ...
                  {profile && (
                    <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm">
                      <div className="flex items-center gap-2 mb-4">
                        <Flame className="w-4 h-4 text-orange-500" />
                        <span className="font-bold text-sm text-zinc-900">Streak Calendar</span>
                        <span className="ml-auto text-xs text-zinc-400">Last 30 days</span>
                      </div>
                      ...
                      <div className="mt-3 flex items-center gap-6 text-xs text-zinc-500">
                        <span>
                          <strong className="text-zinc-800">{profile.student.currentStreak}</strong> current streak
                        </span>
                        <span>
                          <strong className="text-zinc-800">{profile.student.maxStreak}</strong> max streak
                        </span>
                        {profile.stats.globalRank && (
                          <span>
                            Global rank: <strong className="text-zinc-800">#{profile.stats.globalRank}</strong>
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {profile?.recentXp?.length > 0 && (
                    <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="font-bold text-sm text-zinc-900">Recent XP Activity</span>
                      </div>
                      <div className="space-y-2">
                        {profile.recentXp.slice(0, 8).map((tx: any, i: number) => (
                          <div key={i} className="flex items-center justify-between py-2 border-b border-zinc-50 last:border-0">
                            <span className="text-xs text-zinc-600">{tx.reason}</span>
                            <span className="text-xs font-bold text-emerald-600">+{tx.amount} XP</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              {tab === "leaderboard" && (
                <div className="space-y-5">
                  {/* Timeframe selector */}
                  <div className="flex items-center gap-2">
                    {(["WEEKLY", "MONTHLY", "ALL_TIME"] as LbTimeframe[]).map((tf) => (
                      <button
                        key={tf}
                        onClick={() => { setLbTimeframe(tf); loadLeaderboard(tf); }}
                  ...
                  {myLbEntry && (
                    <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 flex items-center gap-4">
                      <div className="text-2xl font-black text-blue-600">#{myLbEntry.rank}</div>
                      <div>
                        <div className="text-sm font-bold text-zinc-900">Your rank</div>
                        <div className="text-xs text-zinc-500">{myLbEntry.totalXp.toLocaleString()} XP</div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-white border border-zinc-200 rounded-2xl overflow-hidden shadow-sm">
                    <div className="grid grid-cols-12 gap-2 px-5 py-3 border-b border-zinc-100 bg-zinc-50 text-[10px] uppercase font-bold text-zinc-400 tracking-wider">
                    ...
                    {lbLoading ? (
                      ...
                      <div className="py-10 text-center text-zinc-400 text-xs">No data yet for this period.</div>
                    ) : (
                          ...
                          >
                            <div className="col-span-1 flex items-center">
                              {RankIcon ? (
                                <RankIcon
                                  className={`w-4 h-4 ${
                                    entry.rank === 1 ? "text-yellow-500" : entry.rank === 2 ? "text-zinc-400" : "text-amber-700"
                                  }`}
                                />
                              ) : (
                                <span className="text-xs font-bold text-zinc-500">{entry.rank}</span>
                              )}
                            </div>
                            <div className="col-span-5 flex items-center gap-2">
                              {entry.avatarUrl ? (
                                <img src={entry.avatarUrl} alt="" className="w-7 h-7 rounded-xl object-cover shrink-0" />
                              ) : (
                                <div className="w-7 h-7 rounded-xl bg-zinc-200 flex items-center justify-center text-zinc-600 text-[10px] font-bold shrink-0">
                                  {entry.name?.[0]}
                                </div>
                              )}
                              <span className={`text-sm font-semibold ${isMine ? "text-blue-700" : "text-zinc-800"}`}>
                                {entry.name}
                              </span>
                              {isMine && (
                            ...
                            <div className="col-span-3 flex items-center justify-end">
                              <span className="text-sm font-bold text-amber-600">{entry.totalXp.toLocaleString()}</span>
                            </div>
              ...
              {tab === "contests" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-zinc-400">{contests.length} contests</span>
                  </div>

                  {contests.length === 0 ? (
                    <div className="bg-white border border-zinc-200 rounded-2xl py-16 text-center text-zinc-400 text-sm shadow-sm">
                      No contests scheduled yet.
                    </div>
                  ) : (
                    contests.map((c: any) => (
                      <div key={c.id} className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            ...
                            <div className="font-bold text-zinc-900 text-sm">{c.title}</div>
                            ...
                            <div className="flex items-center gap-4 mt-2 text-[11px] text-zinc-400">
                              ...
                              <span className="flex items-center gap-1">
                                <Users className="w-3 h-3" /> {c._count?.participants || 0} registered
                              </span>
                              <span className="flex items-center gap-1">
                                <Calendar className="w-3 h-3" /> {new Date(c.startTime).toLocaleDateString()}
                              </span>
                            </div>
                          </div>
                          <button
                            onClick={async () => {
                              if (!studentId) return;
                              try {
                                await apiFetch(`/competitive/contests/${c.id}/register`, { method: "POST" });
                                loadData();
                              } catch {}
                            }}
                            className={`shrink-0 px-4 py-2 text-xs font-bold rounded-xl border transition-all cursor-pointer ${
                              c.status === "ENDED"
                                ...
                                : "bg-[#111111] hover:bg-black text-white border-transparent"
                            }`}
                            disabled={c.status === "ENDED"}
                          >
                            {c.status === "ENDED" ? "Ended" : c.status === "LIVE" ? "Join Now" : "Register"}
                          </button>
                        </div>
                      </div>
                    ))
              ...
              {tab === "achievements" && (
                <div className="space-y-5">
                  <div className="flex items-center justify-between">
                    <h2 className="text-lg font-bold text-zinc-900">Achievements</h2>
                    {profile && (
                      <span className="text-xs text-zinc-500">
                        <strong className="text-zinc-800">{profile.achievements?.length || 0}</strong>{" "}
                  ...
                  {(() => {
                    const unlockedKeys = new Set(
                      (profile?.achievements || []).map((a: any) => a.achievement?.key),
                    );
                            ...
                            return (
                              ...
                              >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                                  unlocked ? "bg-amber-100 border border-amber-300" : "bg-zinc-100 border border-zinc-200"
                                }`}>
                                  {unlocked ? (
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className={`text-sm font-bold truncate ${unlocked ? "text-zinc-900" : "text-zinc-500"}`}>
                                    {a.title}
                                  </div>
                                  <div className="text-[11px] text-zinc-400 truncate">{a.description}</div>
                                  <div className={`text-[10px] font-bold mt-0.5 ${unlocked ? "text-amber-600" : "text-zinc-400"}`}>
                                    +{a.xpReward} XP
                                  </div>
                                </div>
                              </div>
